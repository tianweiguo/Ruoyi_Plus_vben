...
export function listToTree<T = any>(list: any[], config: Partial<TreeHelperConfig> = {}): T[] {
  ...
  removeEmptyChildren(result, config.children);
  return result;
}
...
function removeEmptyChildren(data: any[], childrenField = 'children') {
  data.forEach((item) => {
...
/**
 * 添加全名 如 祖先节点-父节点-子节点
 * @param deptTreeData 已经是tree数据
 * @param labelName 标签的字段名称
 * @param splitStr 分隔符
 * @returns void 无返回值 会修改原始数据
 */
export function addFullName(treeData: any[], labelName = 'label', splitStr = '-') {
...
export function showNodeState(association: boolean): void {
  const state = association ? '节点关联' : '节点独立';
  createMessage.info('当前节点状态为:' + state);
}
