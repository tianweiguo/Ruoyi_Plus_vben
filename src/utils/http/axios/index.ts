  ...
  beforeRequestHook: (config, options) => {
    ...
    if (config.method?.toUpperCase() === RequestEnum.GET) {
      if (!isString(params)) {
        ...
        // 由于ruoyi的日期选择为params[beginTime]格式, 需要进行encode
        // 直接将参数格式化后拼接到url上  不再使用params
        const encodeParams = qs.stringify(params);
        config.url = encodeParams ? `${config.url}?${encodeParams}` : config.url;
    ...
    config.headers['clientid'] = globSetting.clientId;
    // 语言相关
    config.headers['Content-Language'] = localeStore.getLocale;
    config.headers['Accept-Language'] = localeStore.getLocale;
    // 需要加密 && post/put请求
    if (config.encrypt && ['POST', 'PUT'].includes(config.method?.toUpperCase() || '')) {
